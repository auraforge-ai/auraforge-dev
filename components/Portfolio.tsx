  ...
  <div className={`group relative overflow-hidden rounded-lg bg-brand-surface border border-white/10 transition-shadow duration-300 ${project.isLive ? 'animate-card-glow' : 'hover:shadow-xl hover:shadow-brand-secondary/10'}`}>
    ...
    <div className="absolute bottom-0 left-0 p-6">
      ...
      {project.isLive ? (
        ...
        <span className="inline-block px-4 py-2 text-sm bg-gray-700 text-brand-text-muted font-semibold rounded-md opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-4 group-hover:translate-y-0 cursor-not-allowed">
          In Development
        </span>
      )}
    </div>
